...
# import sequences. Clonality was assessed using changeo in the immcantation framework (using the container) after merging fasta files containing sequences across different times post-vaccination. This is reflected in the column "cloneid_merged"
...
```{r split dataframe based on individual samples, include=FALSE, message=FALSE}
p6m <- all_merged %>%
  subset(., subject == "p6m") %>%
  select(-clone_id) %>%
...
# create trees for specific clone IDs (since en masse clonal tree generation does not work). Clone IDs were selected based on V gene (IGHV4-34), clone size, inclusion of class switching events and differentiation events (naive/ all sequences to plasmablasts)
